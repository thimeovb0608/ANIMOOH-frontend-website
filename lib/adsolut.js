...
function buildWebOrderPayload({ customerId, contactId, deliveryAddressId, lines, reference }) {
  // Shipping is deliberately 0 here: a WebOrder has no shipping-cost field
  // (ShipmentType/Transporter are just references), so a paid shipping charge
  // has to arrive as an extra product line in `lines`.
  // TODO: before wiring api/webhook.js, add a "verzendkosten" article in
  // Adsolut and append it as a line whenever metadata.shippingEUR > 0 —
  // otherwise the ERP total will be short by the shipping amount.
  const totals = orderTotals(lines, 0, null);
